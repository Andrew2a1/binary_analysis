// Code from: https://practicalbinaryanalysis.com/
...
int reorder_shdrs(elf_data_t *elf, inject_data_t *inject)
{
    ...
    skip = 0;
    for (scn = elf_getscn(elf->e, inject->sidx + direction); scn != NULL; scn = elf_getscn(elf->e, inject->sidx + direction + skip))
    {
...
int rewrite_code_section(elf_data_t *elf, inject_data_t *inject)
{
    ...
    while ((scn = elf_nextscn(elf->e, scn)))
    {
        ...
        char *s = elf_strptr(elf->e, shstrndx, shdr.sh_name);
        if (!s)
        {
...
int inject_code(int fd, inject_data_t *inject)
{
    ...
    if ((rewrite_code_section(&elf, inject) < 0) || (rewrite_section_name(&elf, inject) < 0))
    {
        goto fail;
    }
...
int main(int argc, char *argv[])
{
    ...
    code = static_cast<char *>(malloc(len));
    if (!code)
    {
