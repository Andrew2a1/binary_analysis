...
static void mem_to_file(mem_cluster_t *c, ADDRINT entry)
{
    ...
    snprintf(buf, sizeof(buf), "unpacked.0x%jx-0x%jx_entry-0x%jx", c->base, c->base + c->size, entry);
...
{
    return std::any_of(clusters.cbegin(), clusters.cend(),
                       [&target](const auto &cluster) { return cluster.base <= target && target < cluster.base + cluster.size; });
}

static void check_indirect_ctransfer(ADDRINT, ADDRINT target)
{
    mem_cluster_t c;
...
static void queue_memwrite(ADDRINT addr) { saved_addr = addr; }
...
static void log_memwrite(UINT32 size)
{
    ...
    {
        shadow_mem[i].w = true;
        const void *src = reinterpret_cast<void *>(i);
        PIN_SafeCopy(&shadow_mem[i].val, src, 1);
    }
}
 ...
 *****************************************************************************/
static void instrument_mem_cflow(INS ins, void *)
{
    if (INS_IsMemoryWrite(ins))
    {
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, TO_AFUNPTR(queue_memwrite), IARG_MEMORYWRITE_EA, IARG_END);
        if (INS_HasFallThrough(ins))
        {
            INS_InsertPredicatedCall(ins, IPOINT_AFTER, TO_AFUNPTR(log_memwrite), IARG_MEMORYWRITE_SIZE, IARG_END);
        }
        if (INS_IsControlFlow(ins))
        {
            INS_InsertPredicatedCall(ins, IPOINT_TAKEN_BRANCH, TO_AFUNPTR(log_memwrite), IARG_MEMORYWRITE_SIZE, IARG_END);
        }
    }
    if (INS_IsControlFlow(ins) && INS_OperandCount(ins) > 0)
    {
        INS_InsertCall(ins, IPOINT_BEFORE, TO_AFUNPTR(check_indirect_ctransfer), IARG_INST_PTR, IARG_BRANCH_TARGET_ADDR, IARG_END);
    }
}
 ...
 *                               Other functions                             *
 *****************************************************************************/
static bool cmp_cluster_size(const mem_cluster_t &c, const mem_cluster_t &d) { return c.size > d.size; }
...
static void print_clusters()
{
    ...
    for (j = 0; j < clusters.size(); j++)
    {
        ...
        fprintf(logfile, "0x%016jx (%9s) %s%s: ", clusters[j].base, buf, clusters[j].w ? "w" : "-", clusters[j].x ? "x" : "-");
        for (m = 0; m < n; m++)
        {
            fprintf(logfile, "=");
        }
